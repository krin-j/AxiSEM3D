  ...
  connectivityToElementGLL(const eigen::IMatX4_RM& connectivity,
    ...
    for (int ielem = 0; ielem < nelem; ielem++) {
      ...
      for (int inb = 0; inb < neighbours[ielem].rows(); inb++) {
        ...
        sharedDOF(connectivity, ielem, inghb, nodeOrEdgeMe, nodeOrEdgeNb, sharingEdge);
        ...
        if (sharingEdge) {
          ...
          elementGLL(ielem, gNodeIPnt[nodeOrEdgeMe]) = elementGLL(inghb, gNodeIPnt[nodeOrEdgeNb]);
  ...
  buildCommMPI(const eigen::IColX& myL2G_Element,
    ...
    for (int letag = 0; letag < myL2G_Element.size(); letag++) {
      ...
      for (int netag : globalNeighbours[getag]) {
        ...
        int nodeOrEdgeMe = -1, nodeOrEdgeNb = -1;
        bool sharingEdge = false;
        sharedDOF(globalConnectivity, getag, netag, nodeOrEdgeMe, nodeOrEdgeNb, sharingEdge);
